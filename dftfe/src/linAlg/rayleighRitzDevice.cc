...
namespace dftfe
{
  namespace linearAlgebraOperationsDevice
  {
      ...
      const bool           useMixedPrecOverall)
    {
      ...
      //   operatorMatrix.getMPICommunicatorDomain(), "RRGEP Begin");
      //
      ...
      // dftUtils::printCurrentMemoryUsage(
      //   operatorMatrix.getMPICommunicatorDomain(), "RRGEP Project done");
      //
      // compute projected Hamiltonian conjugate HConjProj= X^{T}*HConj*XConj
      //
      dftfe::ScaLAPACKMatrix<dataTypes::number> projHamPar(N,
      ...
      // dftUtils::printCurrentMemoryUsage(
      //   operatorMatrix.getMPICommunicatorDomain(), "RRGEP End");
    }
