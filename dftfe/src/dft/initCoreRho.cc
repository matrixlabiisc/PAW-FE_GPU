        ...
        if (cell->is_locally_owned())
          {
            ...
            std::vector<double> rhoCoreAtom(n_q_points, 0.0);
                ...
                if (isCoreRhoDataInCell)
                  {
                    ...
                    for (dftfe::uInt q = 0; q < n_q_points; ++q)
                      {
                        rhoCoreAtomCell[q]             = rhoCoreAtom[q];
            ...
            for (dftfe::uInt iImageCharge = 0;
                 ...
                 ++iImageCharge)
              {
                ...
                if (isCoreRhoDataInCell)
                  {
                    std::vector<double> &rhoCoreAtomCell =
                      d_rhoCoreAtoms[numberGlobalCharges + iImageCharge]
                                    [cell->id()];
                    std::vector<double> &gradRhoCoreAtomCell =
