...
namespace dftfe
{
    ...
    const bool        reinitializeFastConstraints)
  {
    ...
    d_nuclearChargeQuadratureId        = smearedChargeQuadratureId;
    ...
    const bool        reinitializeFastConstraints)
  {
    ...
    d_rhoCoreCorrectionValuesPtr =
      isCoreRhoValues ? &CoreRhoCorrectionValues : NULL;
    d_rhoCoreRefinedValuesPtr = isCoreRhoValues ? &CoreRhoRefinedValues : NULL;
    d_atomsPtr                = smearedNuclearCharges ? NULL : &atoms;
    d_smearedChargeValuesPtr =
      smearedNuclearCharges ? &smearedChargeValues : NULL;
    d_nuclearChargeQuadratureId        = smearedChargeQuadratureId;
    ...
    distributedCPUVec<double> &rhs)
  {
            ...
            for (dftfe::uInt iSubCell = 0; iSubCell < numSubCells; ++iSubCell)
              {
                ...
                const std::vector<double>  zeroCore(fe_eval_density.n_q_points,
                                                   0.0);
                const std::vector<double> *tempCoreVecPtr = &zeroCore;
                if (d_isCoreRhoVals && d_coreRhoValuesPtr != nullptr)
                  {
                    const auto itCore = d_coreRhoValuesPtr->find(subCellId);
                    if (itCore != d_coreRhoValuesPtr->end())
                      tempCoreVecPtr = &(itCore->second);
                  }
                const std::vector<double> *valueCorrectionPtr = &zeroCore;
                if (d_isCoreRhoVals && d_rhoCoreCorrectionValuesPtr != nullptr)
                  {
                    const auto itCorr =
                      d_rhoCoreCorrectionValuesPtr->find(subCellId);
                    if (itCorr != d_rhoCoreCorrectionValuesPtr->end())
                      valueCorrectionPtr = &(itCorr->second);
                  }
                for (dftfe::uInt q = 0; q < fe_eval_density.n_q_points; ++q)
                  {
                    rhoQuads[q][iSubCell] =
                      tempVec[q] +
                      ((*tempCoreVecPtr)[q] - (*valueCorrectionPtr)[q]);
                  }
              }
    ...
    if (d_atomsPtr != NULL)
             ...
             !d_isReuseSmearedChargeRhs)
      {
        ...
        //
        d_matrixFreeDataPtr->get_quadrature(d_nuclearChargeQuadratureId).size();

        dealii::FEEvaluation<3, -1> fe_eval_sc(*d_matrixFreeDataPtr,
                                               d_matrixFreeVectorComponent,
                                               d_nuclearChargeQuadratureId);
                ...
                for (dftfe::uInt iSubCell = 0; iSubCell < numSubCells;
                     ++iSubCell)
                  {
                    subCellPtr = d_matrixFreeDataPtr->get_cell_iterator(
                      macrocell, iSubCell, d_matrixFreeVectorComponent);
                    dealii::CellId subCellId = subCellPtr->id();
                    const auto     itSmeared =
                      d_smearedChargeValuesPtr->find(subCellId);
                    if (itSmeared == d_smearedChargeValuesPtr->end())
                      continue;
                    const std::vector<double> &tempVec = itSmeared->second;
                    if (tempVec.size() == 0)
                      continue;
                    const std::vector<double>  zeroCore(numQuadPointsSmearedb,
                                                       0.0);
                    const std::vector<double> *tempRhoCorePtr = &zeroCore;
                    if (d_isCoreRhoVals && d_rhoCoreRefinedValuesPtr != nullptr)
                      {
                        const auto itCore =
                          d_rhoCoreRefinedValuesPtr->find(subCellId);
                        if (itCore != d_rhoCoreRefinedValuesPtr->end())
                          tempRhoCorePtr = &(itCore->second);
                      }
                    for (dftfe::uInt q = 0; q < numQuadPointsSmearedb; ++q)
                      smearedbQuads[q][iSubCell] =
                        (tempVec[q] + (*tempRhoCorePtr)[q]);
                    ...
                    if (d_isStoreSmearedChargeRhs)
                      {
                        ...
                        fe_eval_sc.distribute_local_to_global(
                          d_rhsSmearedCharge);
                      }
                  }
              }
